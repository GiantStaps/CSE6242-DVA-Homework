...
class Graph:
    ...
    def add_node(self, id: str, name: str) -> None:
        ...
        if not isinstance(id, str):
            raise TypeError("The variable 'source' is not a string.")
        if not isinstance(name, str):
            raise TypeError("The variable 'target' is not a string.")
        name.replace(",", "")
...
if __name__ == "__main__":
    ...
    api_key = '0952c64efc9500a3a556268cbce792c0'
    tmdb_api_utils = TMDBAPIUtils(api_key='0952c64efc9500a3a556268cbce792c0')
    ...
    for movie_credit in movie_credits:
        ...
        for cast_member in movie_cast:
            ...
            if cast_member_id not in new_nodes:
                graph.add_node(cast_member_id, cast_member_name)
                new_nodes.add(cast_member_id)
                graph.add_edge('2975', cast_member_id)
        print(f"Total # Nodes: {graph.total_nodes()}, Total # Edges: {graph.total_edges()}, Time Elapsed: {time.time() - start_time : .2f}")
    ...
    for _ in range(2):
        ...
        for node_id in current_new_nodes:
            try:
            ...
            for movie_credit in movie_credits:
                ...
                for cast_member in movie_cast:
                    ...
                    if cast_member_id not in graph.nodes:
                        graph.add_node(cast_member_id, cast_member_name)
                        new_nodes.add(cast_member_id)

                    graph.add_edge(node_id, cast_member_id)
        print(f"Total # Nodes: {graph.total_nodes()}, Total # Edges: {graph.total_edges()}, Time Elapsed: {time.time() - start_time : .2f}")
